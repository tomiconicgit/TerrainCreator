...
function rebuildGridLines(terrainGroup, terrainMesh, config) {
  ...
  const blueGeom = new THREE.EdgesGeometry(terrainMesh.geometry, 15);
  ...
  // --- THE FIX: ADD THE MISSING ROTATION ---
  // This aligns the red grid with the flat terrain.
  mainGridGeom.rotateX(-Math.PI / 2);
  ...
  for (let j = 0; j <= TILES_Y; j++) {
    for (let i = 0; i <= TILES_X; i++) {
      ...
      // Lift the red grid slightly to prevent z-fighting

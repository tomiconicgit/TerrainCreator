...
(async () => {
  ...
  try {
    ...
    showErrorOverlay('Failed to import "three" from import map (check vendor/three.module.js path and MIME type).', e);
    throw e;
  }
  try {
    ({ Sky } = await import('../vendor/three.sky.js'));
  } catch (e) {
    showErrorOverlay('Failed to import Sky (check vendor/three.sky.js encoding and path).', e);
    throw e;
  }
  ...
  function _fbm(x, y, o = 5, l = 2, g = 0.5, noise = _perlin2) { let a = 1, f = 1, s = 0, n = 0; for (let i = 0; i < o; i++) { s += a * noise(x * f, y * f); n += a; a *= g; f *= l; } return s / n; }
  ...
  function applyTemplate(name) {
    ...
    for (let jy = 0; jy <= ySeg; jy++) {
      ...
      for (let ix = 0; ix <= xSeg; ix++) {
        ...
        switch (name) {
          case 'Flat': n = -1; break;
          case 'DiamondSquare': n = Math.abs(_fbm(u * 2.5, v * 2.5, 5, 2, 0.5)) * 2 - 1; break;
          case 'Perlin': n = _fbm(u * 2.5, v * 2.5, 5, 2, 0.5, _perlin2); break;
          case 'Simplex': n = _fbm(u * 2.8, v * 2.8, 6, 2.1, 0.5, _perlin2); break;
          case 'Fault': n = _fault(u * 2.5, v * 2.5, 64); break;
          case 'Cosine': n = Math.cos(_fbm(u * 2.0, v * 2.0, 4, 2, 0.5) * Math.PI); break;
          case 'Value': n = _fbm((u * 2.5 | 0) + 0.001, (v * 2.5 | 0) + 0.001, 3, 2, 0.6, _perlin2); break;
  ...
  function applyTileDelta(ci, cj, dir, rTiles, step) {
    ...
    posAttr.needsUpdate = true; terrainMesh.geometry.computeVertexNormals(); rebuildEdges(); if (ball) ball.refresh();
  }
  ...
  randBtn.addEventListener('click', () => {
    ...
    terrainMesh.geometry.attributes.position.needsUpdate = true;
    terrainMesh.geometry.computeVertexNormals();
    rebuildEdges(); if (ball) ball.refresh();
  });
  ...
  // ---- Boot / Loop / SW ----
  buildTerrain();
  updateSky(100 * TILE_SIZE, new THREE.Vector3());

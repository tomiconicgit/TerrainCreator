...
function applySculpt(hitPoint, appState, uiState) {
    ...
    // This part requires access to the rebuildEdges function, which is tricky.
    // A simple solution is to have the main loop handle it, or pass it in.
    // For now, we'll assume a global event or direct call is made.
...
export function initTapToMove(appState, getAllowTapMove) {
    const { renderer, camera } = appState;
    renderer.domElement.addEventListener('pointerdown', (ev) => {
        if (!getAllowTapMove()) return;
